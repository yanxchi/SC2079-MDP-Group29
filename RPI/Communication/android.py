...
class Android(Link):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.hostId = "192.168.29.1"
        # UUID to be generated, but can just use the default one
        self.uuid = (
    ...
    def connect(self):
        ...
        try:
            ...
            # port = 2 # port 1 is commonly used, but we already specified port 1 as default when setting up RFCOMM.
            self.server_socket = bt.BluetoothSocket(bt.RFCOMM)
            # ~ self.server_socket.bind((self.hostId, bt.PORT_ANY))
            self.server_socket.bind((self.hostId, 2))
    ...
    def disconnect(self):
        ...
        try:
            ...
            # socket.shutdown is not necessary to close the connection, but is beneficial when dealing with multithreading processes.
    ...
    def send(self, message: AndroidMessage):
        ...
        try:
            # Default code to send a message to Android.
    ...
    def receive(self) -> Optional[str]:
        ...
        try:
            # ~ while True:
            # Default code to receive data from Android in JSON format.
